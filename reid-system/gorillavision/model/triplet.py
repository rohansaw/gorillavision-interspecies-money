...
from gorillavision.utils.batch_sampler_ensure_positives import (
    BatchSamplerEnsurePositives,
)
...
class TripletLoss(pl.LightningModule):
    def __init__(
        ...
        if backbone == "inception":
            self.backbone = create_inception_model(
    ...
    def forward(self, x: Tensor):
        x, attn_weights = self.backbone(x)
    ...
    def prepare_data(self):
        ...
        if self.train_val_split_overlapping:
            train, validate = train_test_split(
                ...
                test_size=0.3,
                random_state=0,
                stratify=self.df["labels_numeric"],
            )
        else:
            train, validate = train_val_split_distinct(
                self.df, test_size=0.3, random_state=0, label_col_name="labels_numeric"
            )
        ...
        if self.sampler == "class_sampler":
            ...
            self.batch_sampler_train = BatchSamplerEnsurePositives(
                ds=self.train_ds, batch_size=self.batch_size
            )
            self.batch_sampler_val = BatchSamplerEnsurePositives(
                ds=self.validate_ds, batch_size=self.batch_size
            )
    ...
    def train_dataloader(self):
        if self.sampler == "class_sampler":
            return DataLoader(
                self.train_ds, batch_sampler=self.batch_sampler_train, num_workers=8
            )
    ...
    def val_dataloader(self):
        if self.sampler == "class_sampler":
            return DataLoader(
                self.validate_ds, batch_sampler=self.batch_sampler_val, num_workers=8
            )
        elif self.sampler == "random_sampler":
            return DataLoader(
                self.validate_ds,
