...
class ImageService:
    def __init__(self, input_dataset_path, output_root, allowed_img_extensions, allowed_video_extensions):
        ...
        self.class_iters = {cls: iter([img for img in cls.iterdir() if img.suffix.lower().endswith(tuple(allowed_img_extensions + allowed_video_extensions))]) for cls in self.classes}
    ...
    def validate_data(self, allowed_img_extensions, allowed_video_extensions):
        ...
        for folder in sub_folder:
            ...
            if not any(file.lower().endswith(tuple(allowed_img_extensions + allowed_video_extensions)) for file in files):
        ...
        for folder in os.listdir(self.input_dataset_path):
            ...
            for file in os.listdir(os.path.join(self.input_dataset_path, folder)):
                if self.image_already_processed(Path(file), folder):
                    logging.info(f"File {file} in class {folder} already processed. Skipping.")
                    continue
                shutil.copy(os.path.join(self.input_dataset_path, folder, file), os.path.join(output_original_files, folder, file))
    ...
    def image_already_processed(self, image, folder):
        ...
        def check_if_file_in_path(file_path):
            return len(list(filter(lambda name: image.stem in name, os.listdir(file_path)))) > 0 if os.path.exists(file_path) else False
        return check_if_file_in_path(original_path) and check_if_file_in_path(label_path)
